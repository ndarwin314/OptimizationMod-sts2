...
public class Serializable
{
    ...
    private class CardTuple : IPacketSerializable
    {
    ...
    { 
        ...
        [HarmonyPrefix]
        public static bool Serialize(PacketWriter writer, SerializablePlayer __instance)
        {
        ...
        [HarmonyPrefix]
        public static bool Deserialize(PacketReader reader, SerializablePlayer __instance)
        {
            // if not using modified save format, fall back to default implementation
            if (_schemaVersion != 99) return true;
    ...
    [HarmonyPatch(typeof(SerializableRun))]
    public class RunSerializer
    {
        
        [HarmonyPatch(nameof(SerializableRun.Serialize))]
        [HarmonyPrefix]
        public static void Serialize(SerializableRun __instance)
        {
            // hacky way to identify a different schema version since i don't want to deal with doing that
            // the correct way right now
            __instance.SchemaVersion = 99;
        }
        
        [HarmonyPatch(nameof(SerializableRun.Deserialize))]
        [HarmonyPrefix]
        public static void Deserialize(PacketReader reader)
        {
           _schemaVersion = ReadIntNoAdvance(reader);
        }
